...
class PlayerRatingCalculator implements PlayerRSCalculatorContract
{
    ...
    public function calculate(string $expression, Player $originalPlayer, array $serverIds): ?float
    {
        ...
        $variablesForRatingDynamic = array_keys(config('constants.variables_for_rating_dynamic'));

        $variablesForRatingDynamicParsed = [];
        $serverIds = array_unique($serverIds);
        foreach ($serverIds as $serverId) {
            foreach ($variablesForRatingDynamic as $variable) {
                $variablesForRatingDynamicParsed[] = str_replace('{id}', $serverId, $variable);
            }
        }
