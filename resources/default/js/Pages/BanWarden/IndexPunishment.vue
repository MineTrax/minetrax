...
<template>
 ...
 <Card>
 <CardContent class="flex flex-row p-4">
 <div class="flex items-center justify-center shrink-0 h-12 w-12 rounded-xl bg-sky-100 dark:bg-sky-500/10 text-sky-600 dark:text-sky-400">
 ...
 <div class="font-bold text-card-foreground text-lg">
 ...
 <Card>
 <CardContent class="flex flex-row p-4">
 <div class="flex items-center justify-center shrink-0 h-12 w-12 rounded-xl bg-destructive/10 text-destructive">
 ...
 <div class="font-bold text-lg text-card-foreground">{{ metrics.total_active_bans }} / {{ metrics.total_bans }}</div>
 </div>
 </CardContent>
 </Card>
 </div>
 ...
 <Card>
 <CardContent class="flex flex-row p-4">
 <div class="flex items-center justify-center shrink-0 h-12 w-12 rounded-xl bg-muted dark:bg-muted text-muted-foreground">
 ...
 <div class="font-bold text-lg text-card-foreground">{{ metrics.total_active_mutes }} / {{ metrics.total_mutes }}</div>
 </div>
 </CardContent>
 </Card>
 </div>
 ...
 <Card>
 <CardContent class="flex flex-row p-4">
 <div class="flex items-center justify-center shrink-0 h-12 w-12 rounded-xl bg-yellow-100 dark:bg-yellow-500/10 text-yellow-600 dark:text-yellow-400">
 ...
 <div class="font-bold text-lg text-card-foreground">{{ metrics.total_active_warns }} / {{ metrics.total_warns }}</div>
 </div>
 </CardContent>
 </Card>
 </div>
 ...
 <Card>
 <CardContent class="flex flex-row p-4">
 <div class="flex items-center justify-center shrink-0 h-12 w-12 rounded-xl bg-orange-100 dark:bg-orange-500/10 text-orange-600 dark:text-orange-400">
 ...
 <DataTable class="bg-card border rounded-lg shadow w-full":header="headerRow":data="punishments":filters="filters">
 ...
 <div v-tippy class="shrink-0 h-8 w-8 focus:outline-hidden":content="item.country.name">
 ...
 >
 <span v-if="item.victim_player.username"class="font-extrabold text-foreground dark:text-foreground">{{ item.victim_player.username }}</span>
 <span v-else class="text-destructive italic">{{ __("Unknown") }}</span>
 </Link>
 </div>
 </div>
 </div>
 ...
 <div class="ml-2">
 <div v-tippy class="text-sm font-medium text-foreground dark:text-foreground":content="item.uuid">
 <span class="text-destructive italic">{{ __("Unknown") }}</span>
 </div>
 </div>
 </div>
 </div>
 ...
 <span class="text-foreground dark:text-foreground text-xs font-bold">{{ __("IP :punish", { punish: item.type.key }) }} </span>
 </div>
 </td>

 <DtRowItem v-if="canShowMaskedIp"class="text-center">
 <span v-if="item.ip_address || item.masked_ip_address">
 <a class="hover:underline"target="_blank":href="`https://check-host.net/ip-info?host=${item.ip_address || item.masked_ip_address}`">{{ item.ip_address || item.masked_ip_address }} </a>
 </span>
 ...
 >
 <span v-if="item.creator_player?.username || item.creator_username"class="font-extrabold text-foreground dark:text-foreground">{{
 ...
 <span v-if="item.creator_username && item.creator_username !=='Console'"class="text-foreground dark:text-foreground text-xs font-bold">{{ item.creator_username }} </span>
 <span v-else class="text-foreground dark:text-foreground text-xs font-bold">{{ __("CONSOLE") }} </span>
 </div>
 </td>
 ...
 <InertiaLink v-tippy :title="__('View details')"as="a":href="route('player.punishment.show', item.id)"class="inline-flex items-center justify-center text-primary hover:text-primary">
