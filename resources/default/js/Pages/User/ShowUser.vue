...
"
 ...
 :content="role.display_name"class="focus:outline-hidden max-h-8"/>
 <div v-else
...
"class="text-lg font-bold text-primary hover:text-primary/80">
 ...
 <icon v-if="player.rating != null"v-tippy class="w-8 h-8 focus:outline-hidden"
...
"v-tippy :src="player.rank.photo_url":alt="player.rank.name"
 :title="player.rank.name"class="max-h-12 max-w-12 focus:outline-hidden"/>
 </div>
 </div>
 ...
 <div class="flex items-center space-x-2">
 <p v-tippy class="focus:outline-hidden text-card-foreground":title="formatToDayDateString(
 player.last_seen_at
 )
">
 {{
...
"class="flex justify-between">
 ...
 <span v-if="profileUser.email_verified_at"class="text-success text-xs">
