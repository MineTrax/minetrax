...
const breadcrumbItems = [
    ...
    {
        text: __("Edit Variable"),
        current: false,
    },
    {
        text: "#" + props.storeVariable.id,
        current: true,
    }
];
...
const form = useForm({
    name: props.storeVariable.name,
    identifier: props.storeVariable.identifier,
    description: props.storeVariable.description || "",
    type: props.storeVariable.type?.value ?? props.storeVariable.type,
    options: props.storeVariable.options,
    placeholder: props.storeVariable.placeholder,
    is_required: !! props.storeVariable.is_required,
    max_length: props.storeVariable.max_length,
    is_enabled: !! props.storeVariable.is_enabled,
    sort_order: props.storeVariable.sort_order,
});
...
function updateVariable() {
    form.put(route("admin.store.variable.update", props.storeVariable.id), {});
}
</script>

<template>
  <AdminLayout>
    <app-head :title="__('Edit Store Variable #:id', { id: storeVariable.id })" />
            ...
            <div class="px-4 py-5 bg-card sm:p-6 border-b border-border">
              <div class="grid grid-cols-6 gap-6">
                ...
                <div class="col-span-6 sm:col-span-3">
                  ...
                  <p
                    v-if="storeVariable.packages_count > 0 && form.identifier !== storeVariable.identifier"
                    class="text-xs text-yellow-600 dark:text-yellow-400 mt-2"
                  >
                    {{ __("This variable is used by :count package(s). Renaming the identifier changes the placeholder, so update those commands too.", { count: storeVariable.packages_count }) }}
                  </p>
                  <p
