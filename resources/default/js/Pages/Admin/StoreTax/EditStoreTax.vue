...
const props = defineProps({
    ...
    storeTax: { type: Object, required: true },
});
...
const breadcrumbItems = [
    ...
    { text: "#" + props.storeTax.id, current: true },
];

const form = useForm({
    name: props.storeTax.name,
    country_id: props.storeTax.country_id === null ? GLOBAL : String(props.storeTax.country_id),
    rate_percent: Number(props.storeTax.rate_bp) / 100,
    is_inclusive: props.storeTax.is_inclusive,
    is_enabled: props.storeTax.is_enabled,
});

const submit = () => {
    form.transform(toPayload).put(route("admin.store.tax.update", props.storeTax.id));
};
</script>

<template>
  <AdminLayout>
    <app-head :title="__('Edit Tax')" />
