...
<template>
  <AdminLayout>
    ...
    <Dialog
      :open="showingDetailsModel"
      @update:open="showingDetailsModel = $event"
    >
      <DialogContent class="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div class="w-full text-sm grid grid-cols-2 gap-4 py-4">
          ...
          <div class="col-span-2">
            ...
            <code
              v-if="selectedCommandQueue.output"
              class="block bg-muted px-3 py-2 rounded text-sm font-mono break-all"
            >
          ...
          <div>
            ...
            <div class="flex items-center gap-2">
              <span
                v-if="selectedCommandQueue.last_attempt_at"
                v-tippy
                class="whitespace-nowrap"
          ...
          <div>
            ...
            <div>
              <span
                v-if="selectedCommandQueue.execute_at"
                v-tippy
          ...
          <div>
            ...
            <div
              v-if="selectedCommandQueue.player_id"
              class="flex items-center gap-2"
            >
              <img
                class="w-8 h-8 rounded"
                :src="selectedCommandQueue.player.avatar_url"
                alt=""
              >
              <Link
                v-tippy
                as="a"
                :href="route('player.show', selectedCommandQueue.player.uuid)"
                class="font-medium hover:underline"
                :content="selectedCommandQueue.player.uuid"
              >
                <span
                  v-if="selectedCommandQueue.player.username"
                  class="font-semibold"
                >
          ...
          <div>
            ...
            <div v-if="selectedCommandQueue.user_id">
              <Link :href="route('user.public.get', selectedCommandQueue.user.username)">
                <UserDisplayname
                  :user="selectedCommandQueue.user"
                  icon-class="w-4 h-4"
                  text-class="text-sm"
                />
              </Link>
          ...
          <div class="col-span-2">
            ...
            <code
              v-if="selectedCommandQueue.config"
              class="block bg-muted px-3 py-2 rounded text-sm font-mono break-all"
            >
        ...
        <DialogFooter>
          <Button
            variant="outline"
            @click="showingDetailsModel = false"
          >
            {{ __("Close") }}
          </Button>
        </DialogFooter>
