<template>
    <div :class="{
        'bg-accent rounded': notification.read_at == null,
    }" @click="markAsRead">
        <inertia-link v-if="
            notification.type === 'App\\Notifications\\PostCommentedByUser'
        " as="div" :href="route('post.show', notification.data.post_id)" class="flex cursor-pointer">
            <img :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
        ...
        <inertia-link v-if="notification.type === 'App\\Notifications\\PostLikedByUser'" as="div"
            :href="route('post.show', notification.data.post_id)" class="flex cursor-pointer">
            <img :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
                class="w-10 h-10 rounded-full m-1">
            <div class="m-1">
                <p class="text-foreground">
                    <b>{{ notification.data.causer.name }}</b>(@{{ notification.data.causer.username }}) liked your
                    post.
                </p>
                <p class="text-xs text-muted-foreground">
                    {{ formatTimeAgoToNow(notification.created_at) }}
        ...
        <inertia-link v-if="notification.type === 'App\\Notifications\\UserYouAreMuted'" as="div"
            :href="route('profile.show')" class="flex cursor-pointer">
            <img :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
                class="w-10 h-10 rounded-full m-1">
            <div class="m-1">
                <p class="text-foreground">
        ...
        <inertia-link v-if="notification.type === 'App\\Notifications\\UserYouAreBanned'" as="div"
            :href="route('profile.show')" class="flex cursor-pointer">
            <img :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
                class="w-10 h-10 rounded-full m-1">
            <div class="m-1">
        ...
        " as="div" :href="route('admin.custom-form-submission.show', notification.data.id)
            " class="flex cursor-pointer">
            <img v-if="notification.data.causer" :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
                class="w-10 h-10 rounded-full m-1">
            <img v-else src="/images/steve.png" alt="Anonymous" class="w-10 h-10 rounded-full m-1">
            <div class="m-1">
                <p class="text-foreground">
        ...
        " as="div" :href="route('news.show', notification.data.news.slug)" class="flex cursor-pointer">
            <img :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
                class="w-10 h-10 rounded-full m-1">
            <div class="m-1">
                <p class="text-foreground">
        ...
        " as="div" :href="route('admin.recruitment-submission.show', notification.data.id)
            " class="flex cursor-pointer">
            <img :src="notification.data.causer.profile_photo_url" alt="Profile Picture"
                class="w-10 h-10 rounded-full m-1">
            <div class="m-1">
        ...
        " as="div" :href="notification.data.for_staff
            ? route(
