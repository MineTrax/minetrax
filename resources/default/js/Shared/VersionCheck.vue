    ...
    <AlertCard variant="destructive">
      ...
      <template #body>
        <div class="text-card-foreground">
          {{ __("Your current MineTrax version is") }}&nbsp;<span class="text-destructive font-semibold">{{ response.my_version }}</span>&nbsp;{{ __("while the latest version is") }}&nbsp;<span class="text-success font-semibold">{{ response.latest_version }}</span>. {{ __("Please upgrade to enjoy latest features.") }}
          <br><br>
          <a
