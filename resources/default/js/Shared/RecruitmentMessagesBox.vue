...
onMounted(async () => {
    ...
    messageQueryInterval.value = setInterval(() => pollServerForNewMessages(), 5000);
});
...
const addUniqueMessages = (newMessagesList) => {
    const uniqueMessages = newMessagesList.filter((newMsg) => !messages.value.some((msg) => msg.id === newMsg.id));
...
const pollServerForNewMessages = () => {
    const afterId = messages.value.length > 0 ? messages.value[messages.value.length - 1].id : 0;
...
<template>
  <div class="flex flex-col p-4 space-y-4">
    ...
    <AlertCard
      ...
      variant="destructive"
    >
      {{ __("Messages feature is disabled for this recruitment. Applicant can't send messages.") }}
    </AlertCard>
    ...
    >
      ...
      >
        ...
        >
          <template v-if="comment.type.value === 'recruitment_action'">
            <div class="italic text-sm text-foreground dark:text-foreground">
              <InertiaLink
                as="span"
              ...
              <span class="mr-1">&nbsp;{{ __("changed application status to") }}&nbsp;</span>
              <CommonStatusBadge :status="comment.comment" />
              <span
                v-tippy
                class="inline ml-1 text-xs text-foreground dark:text-foreground focus:outline-hidden"
          ...
          <template v-else>
            ...
            >
            <div
              v-if="comment.type.value === 'recruitment_staff_whisper'"
              v-tippy
              class="flex mt-3.5 ml-1"
              :class="{
                'order-3': comment.type.value === 'recruitment_applicant_message',
              }"
              :title="__('This is a whisper message only visible to staff')"
            >
              <ShieldCheckIcon class="w-5 h-5 text-yellow-400 dark:text-yellow-300" />
            </div>
            <div
            ...
            >
              ...
              >
                <span
                  v-if="comment.is_new"
                  class="absolute -top-1 -right-1 w-3 h-3 bg-destructive rounded-full animate-pulse"
                />
                <InertiaLink
                  as="a"
                ...
                <p
                  class="leading-snug wrap-break-word whitespace-pre-line md:leading-normal"
                  v-html="purifyAndLinkifyText(comment.comment)"
                />
              </div>
            </div>
            <InertiaLink
              v-if="can('delete recruitment_submission_messages') && forAdmin"
              v-confirm="{
              ...
              as="button"
              method="delete"
              :href="route('admin.recruitment-submission.message.delete', [submission.id, comment.id])"
            ...
            >
              <Icon
                name="trash"
                class="w-4 h-4 text-foreground hover:text-destructive dark:text-foreground"
              />
            </InertiaLink>
          </template>
        </div>
    ...
    >
      ...
      <div class="flex justify-end mt-2">
        <LoadingButton
          ...
          v-tippy
          :disabled="!submission.i_can_send_message || isSendingMessage"
          :title="__('Send whisper to other staff members. This message will be private and only visible to staff members.')"
        ...
        <LoadingButton
          ...
          class="px-4 py-2"
          :loading="isSendingMessage"
          @click="forAdmin ? sendMessage('recruitment_staff_message') : sendMessage()"
        >
          <PaperAirplaneIcon
            v-if="!isSendingMessage"
            class="w-5 h-5"
